...
nfl_tidy <-
  ...
  mutate(
    stadium = stadium[which.max(year)], .by = stadium_id
  ) |> 
  mutate(
    game_id = row_number() # create game id for convenience later on
  )
...
In part 2 of this analysis I will dive into some modeling techniques so we can assess more definitively assess the effect these variables have on game attendance. As we learned throughout this post, there are some prominent within-group effects at play and we will be sure to address those in the model development process.

Before ending, let's pin our tidy data to a local versioned board, so that we can read it in Part 2, as well as keep track of any changes we might make to the data in this part.

```{r}
nfl_board <- pins::board_folder(here::here("cleaned_data"), versioned = T)
nfl_tidy_final <- 
  nfl_tidy |> 
  left_join(
    nfl_streaks_simplified |> 
      select(game_id, streak_home_team, streak_away_team)
  ) |> 
  left_join(
    nfl_tidy_with_distances |> 
      select(game_id, distance_between_teams)
  ) |> 
  select(-contains("_streak_")) # remove old streak variables
...
nfl_board |> 
  pins::pin_write(
    nfl_tidy_final, 
    name = "nfl_tidy",
    title = "Tidy NFL Attendance Data", 
    description = "NFL Attendance data with one row per game. Includes other variables; some derived and some pulled directly from other data sets"
  )
```
